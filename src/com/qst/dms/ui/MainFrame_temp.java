...
public class MainFrame_temp extends JFrame {
	...
	private void initToolBar() {
		...
		btnGather = new JButton("采集数据", new ImageIcon("D:\\课程开发工具\\images\\gatherData.png"));
		// 注册监听
		...
		btnMatchLog = new JButton("匹配日志数据", new ImageIcon("D:\\课程开发工具\\images\\matchData.png"));
		// 注册监听
		btnMatchLog.addActionListener(new MatchLogListener());
		toolBar.add(btnMatchLog);

		//添加带有图标的工具栏按钮--->匹配物流数据 ，并注册监听
		ImageIcon matchIcon2 = new ImageIcon("images\\matchData.png");
		btnMatchLog = new JButton("匹配物流数据", new ImageIcon("D:\\课程开发工具\\images\\matchData.png"));
		// 注册监听
		...
		btnSave = new JButton("保存数据", new ImageIcon("D:\\课程开发工具\\images\\saveData.png"));
		// 注册监听
		...
		btnSend = new JButton("发送数据", new ImageIcon("D:\\课程开发工具\\images\\sendData.png"));
		...
		btnShow = new JButton("显示数据", new ImageIcon("D:\\课程开发工具\\images\\showData.png"));
	...
	private class GatherLogListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			log = new LogRec(0, time, adress, type, user, ip, logType);
			// 将日志对象添加到日志列表
			logList.add(log);
			// 显示对话框
	...
	private class GatherTransListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			/*trans = new Transport(id, time, adress, type, handler, reciver,
					transportType);*/
			trans = new Transport(0, time, adress, type, handler, reciver,
					transportType);
			// 将物流对象放入物流列表
			transList.add(trans);
			// 显示对话框
	...
	private class SaveDataListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			// 保存匹配的物流信息
			//若保存成功，弹出提示框：匹配的日志数据已保存到文件和数据库中！",
			//若没有保存成功，则弹出相应的告警提示框
			...
			// 保存匹配的物流信息
			//若保存成功，弹出提示框：匹配的物流数据已保存到文件和数据库中！",
			//若没有保存成功，则弹出相应的告警提示框
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				if (matchedTrans != null && matchedTrans.size() > 0) {
					...
					Socket transSocket = new Socket(serverIP, 6666);
					// 创建用于序列化匹配物流信息对象的输出流
	...
	//线程类，每隔1分钟刷新一次显示数据表格中的数据与数据库保持同步
